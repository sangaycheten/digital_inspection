...
class InspectionSubmittedNotification extends Notification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        return (new MailMessage)
            ->subject("Inspection Submitted – {$client} / {$site}")
            ->greeting("Hello {$notifiable->name},")
            ->line("{$this->technician->name} has submitted {$this->recordCount} inspection record(s) for review.")
            ->line("**Client:** {$client}")
            ->line("**Site:** {$site}")
            ->line("**Building(s):** {$buildings}")
            ->action('Review Inspections', route('reviewer.inspections.index'))
            ->line('Please log in to review and approve the submitted inspections.');
    }
    ...
    public function toDatabase(object $notifiable): array
    {
        ...
        return [
            ...
            'message'       => "{$this->technician->name} submitted {$this->recordCount} inspection record(s) for {$client} / {$site}.",
